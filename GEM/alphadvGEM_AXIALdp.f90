...
!	call MNparm(7,'atau',0.29664d0,0.1d-4,0.0d0,0.0d0,perr)
	...
	open(95, file='DUMMY_AX_43', status='REPLACE')
...
!	call MNparm(7,'atau',0.312d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(1,'alpha0',1.39d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(2,'beta0',1.73d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(3,'gamma0',2.81d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(4,'delta0',3.30d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(5,'gamma1',1.0d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(6,'delta1',1.0d0,0.1d-4,0.0d0,0.0d0,perr)
...
!	call MNparm(7,'atau',0.34d0,0.1d-4,0.0d0,0.0d0,perr)
	...
	arg(2) = 6.0_dp
	arg(3) = 3.0_dp
	!call mnexcm(fcn, 'Set LIMits', (/ 1._dp, 0.0d0, 100.0d0 /), 3, perr, 0)
	!call mnexcm(fcn, 'Set LIMits', (/ 7._dp, 0.28d0, 0.35d0 /), 3, perr, 0)
	call mnexcm(fcn, 'SET STRategy', (/ 2._dp /), 1, perr, 0)
	!call mnexcm(fcn, 'HESse', (/ 2._dp /), 0, perr, 0)
	call mnexcm(fcn,'FIX', (/5.0d0,6.0d0,6.0d0,7.0_dp,12._dp,13._dp/), 2, perr, 0)  
...
!	call MNparm(5,'gamma1',4._dp*pval(3),ierr(3),0.0d0,0.0d0,perr)
	call mnexcm(fcn,'RELeas', (/6.0d0,5.0_dp,6.0_dp,7.0_dp/), 1, perr, 0)
...
!write(*,*) truecovv(58,58)
	...
	!
		...
		do n = binmin, 78
			VFO(n) = IntegralPTFO(n, atau) - ni_DVV(n, a0,b0,c0,d0,c1,d1) - FESRa(n) - 0.0170054
		end do
...
!		/ (cosh(gammaV * s**rhoV) - cos(alphaV * s**rhoV + betaV)) )
	...
	!
		...
		call dqagi(f_DVV,sbin(bin)+dsbin(bin)/2._dp,1,1.0d-12,1.0d-18,ni_DVV,fabserr,fneval, &
	...
	FUNCTION Chi2FOV(a0,b0,c0,d0,c1,d1, atau)
		...
		Chi2FOV = dot_product(FOV(binmin:78),matmul(SigInvA(binmin:78,binmin:78),FOV(binmin:78)))
