MODULE param
	...
	real (kind = dp), dimension(:,:), allocatable :: SigInvV, SigInvA, SigInvVA
	...
	real (kind = dp), dimension(3:78,3:78) :: truecovA, fesrcovA
	real (kind = dp), dimension(0:78,3:78) :: truecovVA
	
	
	real (kind = dp), dimension(3:78) :: fesrA
	
	real (kind = dp), dimension(0:78) :: sbin, spec
	real (kind = dp), dimension(3:78) :: specA
	...
	SUBROUTINE init(bin) ! initialize experimental data arrays and spline data
		...
		open(55, file = 'data/truecovA.dat')
		do n = 3, 78
			read(55,*) truecovA(n,3:78)
		end do
		close(55)

		open(55, file = 'data/truecovVA.dat')
		do n = 0, 78
			read(55,*) truecovVA(n,3:78)
		end do
		close(55)

		open(55, file = 'data/fesrA.dat')
		do n = 3, 78
			read(55,*) fesrA(n)
		end do
		close(55)
		...
		do n=3,78
			fesrcovA(n,n) = dot_product(dsbin(3:n),matmul(truecovA(3:n,3:n),dsbin(3:n)))/(2._dp*pi**2)**2
		end do
		...
		SigInvV(binmin:78,binmin:78) = INVMV()
		
		allocate(SigInvA(binmin:78,binmin:78), stat=statearr)
		
		SigInvA(binmin:78,binmin:78) = INVMA()
		
		allocate(SigInvVA(1:2*(78-binmin+1),1:2*(78-binmin+1)), stat=statearr)
	...
	!
		integer :: i,j
		real (kind=dp), dimension(:,:), allocatable :: M, INVMV
		...
		allocate(INVMV(binmin:78,binmin:78),stat=statearr)
		...
		INVMV(binmin:78,binmin:78) = B(1:78-binmin+1,1:78-binmin+1)
	...
	!
		integer :: i,j
		real (kind=dp), dimension(:,:), allocatable :: M, INVMA
		real (kind=dp), dimension(:,:), allocatable :: A,B
		integer :: INDX(78-binmin+1),D,CODE
		...
		allocate(B(1:78-binmin+1,1:78-binmin+1),stat=statearr)
		allocate(M(binmin:78,binmin:78),stat=statearr)
		allocate(INVMA(binmin:78,binmin:78),stat=statearr)

		do i = binmin, 78
			B(i+1-binmin,i+1-binmin) = 1._dp
			M(i,i) = dot_product(dsbin(3:i),matmul(truecovA(3:i,3:i),dsbin(3:i)))/(2._dp*pi**2)**2
			do j = i+1, 78
				B(i+1-binmin,j+1-binmin) = 0._dp
				B(j+1-binmin,i+1-binmin) = 0._dp
				M(i,j)=dot_product(dsbin(3:i),matmul(truecovA(3:i,3:j),dsbin(3:j)))/(2._dp*pi**2)**2
				M(j,i)=M(i,j)
			end do
		end do
		...
		INVMA(binmin:78,binmin:78) = B(1:78-binmin+1,1:78-binmin+1)
	...
	!
		integer :: i,j
		real (kind=dp), dimension(:,:), allocatable :: MA,MV,MVA,MAV, INVMVA
		real (kind=dp), dimension(:,:), allocatable :: A
		integer :: INDX(2*(78-binmin+1)),D,CODE
		
		allocate(A(1:2*(78-binmin+1),1:2*(78-binmin+1)),stat=statearr)
		allocate(INVMVA(1:2*(78-binmin+1),1:2*(78-binmin+1)),stat=statearr)
		allocate(MV(binmin:78,binmin:78),stat=statearr)
		allocate(MA(binmin:78,binmin:78),stat=statearr)
		allocate(MVA(binmin:78,binmin:78),stat=statearr)
		allocate(MAV(binmin:78,binmin:78),stat=statearr)

		INVMVA = 0.0d0*INVMVA
		forall(i=1:2*(78-binmin+1)) INVMVA(i,i)=1.0d0

		do i = binmin, 78
			MV(i,i) = dot_product(dsbin(0:i),matmul(truecovV(0:i,0:i),dsbin(0:i)))/(2._dp*pi**2)**2
			MA(i,i) = dot_product(dsbin(3:i),matmul(truecovA(3:i,3:i),dsbin(3:i)))/(2._dp*pi**2)**2
			do j = i+1, 78
				MV(i,j)=dot_product(dsbin(0:i),matmul(truecovV(0:i,0:j),dsbin(0:j)))/(2._dp*pi**2)**2
				MV(j,i)=MV(i,j)
				MA(i,j)=dot_product(dsbin(3:i),matmul(truecovA(3:i,3:j),dsbin(3:j)))/(2._dp*pi**2)**2
				MA(j,i)=MA(i,j)
			end do
		end do
		...
		do i = binmin, 78
			do j = binmin, 78
				MVA(i,j) = dot_product(dsbin(0:i),matmul(truecovVA(0:i,3:j),dsbin(3:j)))/(2._dp*pi**2)**2
				MAV(i,j) = MAV(j,i)
			end do
		end do
		
		A(1:78-binmin+1,1:78-binmin+1) = MV(binmin:78,binmin:78)
		A(1:78-binmin+1,1+78-binmin+1:2*(78-binmin+1)) = MVA(binmin:78,binmin:78)
		A(1+78-binmin+1:2*(78-binmin+1),1:78-binmin+1) = MAV(binmin:78,binmin:78)
		A(1+78-binmin+1:2*(78-binmin+1),1+78-binmin+1:2*(78-binmin+1)) = MA(binmin:78,binmin:78)
		
		call ludcmp(A,2*(78-binmin+1),INDX,D,CODE)
		
		do i=1, 2*(78-binmin+1)
			call LUBKSB(A,2*(78-binmin+1),INDX,INVMVA(i,1:2*(78-binmin+1)))
		end do
