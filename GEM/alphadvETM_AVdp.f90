...
!	call MNparm(7,'atau',0.29664d0,0.1d-4,0.0d0,0.0d0,perr)
	...
	fname = 'DUMMY_AV_'//trim(fname)
	
	open(95, file=fname, status='REPLACE')
	!write(95,*) 'DOF, binmin, Energy, (Pval,Perr)[1->7], Chi, P-Value, Converged [0=yes]'	

	call mnexcm(fcn, 'CLEar', (/ 1.0d0 /), 0, perr, 0)

	call MNparm(1,'alpha0_V',0.85d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(2,'beta0_V',1.94d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(3,'gamma0_V',1.53d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(4,'delta0_V',2.64d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(5,'gamma1_V',13.62d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(6,'delta1_V',0.71d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(7,'alpha0_A',3.371d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(8,'beta0_A',0.66d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(9,'gamma0_A',3.74d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(10,'delta0_A',2.67d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(11,'gamma1_A',0.1d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(12,'delta1_A',0.36d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(13,'atau',0.31d0,0.1d-4,0.0d0,0.0d0,perr)	
...
!	call MNparm(7,'atau',0.34d0,0.1d-4,0.0d0,0.0d0,perr)
	...
	arg(2) = 6.0_dp
	arg(3) = 3.0_dp
	!call mnexcm(fcn, 'Set LIMits', (/ 2._dp, 0.01d0, 1.0d0 /), 3, perr, 0)
	call mnexcm(fcn, 'SET STRategy', (/ 2._dp /), 1, perr, 0)
	!call mnexcm(fcn, 'HESse', (/ 2._dp /), 0, perr, 0)
	!call mnexcm(fcn,'FIX', (/5.0d0,6.0d0,11.0_dp,12._dp,13._dp/), 4, perr, 0)  
	!call mnexcm(fcn,'MINimize',    (/10000.0_dp,1.0d-1/),2,perr,0) 
...
!	call MNparm(5,'gamma1',4._dp*pval(3),ierr(3),0.0d0,0.0d0,perr)
	!call mnexcm(fcn,'RELeas', (/5.0_dp,6.0_dp,11.0_dp,12.0_dp/), 2, perr, 0)
	...
	call mnexcm(fcn,'MINimize',    (/10000.0_dp,1.0d0/),2,perr,0)  ! perform MIGRAD minimization
...
!	call mnexcm(fcn, 'HESse', (/ 2._dp /), 0, perr, 0)
	...
	call get_ERR(13, pval, ierr, fval, 1.0d-4		)
