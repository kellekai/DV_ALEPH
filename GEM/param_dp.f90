MODULE param
	...
	SUBROUTINE init(bin) ! initialize experimental data arrays and spline data
		...
		allocate(SigInvV(binmin:78,binmin:78), stat=state)
	...
	!
		...
		allocate(A(1:78-binmin+1,1:78-binmin+1),stat=state)
		allocate(B(1:78-binmin+1,1:78-binmin+1),stat=state)
		allocate(M(binmin:78,binmin:78),stat=state)
		allocate(INVM(binmin:78,binmin:78),stat=state)
