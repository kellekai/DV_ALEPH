PROGRAM main
	...
	open(95, file='RESULT_GEM_FOPT', status='REPLACE')
...
!	call MNparm(1,'alpha',4.19d0,0.1d-4,0.0d0,0.0d0,perr)
!	call MNparm(2,'beta',1.9d-4,0.1d-4,0.0d0,0.0d0,perr)
!	call MNparm(3,'gamma',4.8d0,0.1d-4,0.0d0,0.0d0,perr)
!	call MNparm(4,'delta',0.11d0,0.1d-4,0.0d0,0.0d0,perr)
!	call MNparm(5,'rho',9.3d0,0.1d-4,0.0d0,0.0d0,perr)
!	call MNparm(6,'sigma',3.7d0,0.1d-4,0.0d0,0.0d0,perr)
!	call MNparm(7,'atau',0.319d0,0.1d-4,0.0d0,0.0d0,perr)
!	call mnexcm(fcn,'SET PRINTout', (/1.0_dp/),1,perr,0)  ! set print level (2=full)
	

	call MNparm(1,'alphaV',1.0d0,0.1d0,0.0d0,0.0d0,perr)
	call MNparm(2,'betaV',1.0d0,0.1d0,0.0d0,0.0d0,perr)
	call MNparm(3,'gammaV',1.0d-3,0.1d0,0.0d0,0.0d0,perr)
	call MNparm(4,'deltaV',1.0d0,0.1d0,0.0d0,0.0d0,perr)
...
!	call mnexcm(fcn, 'SCAn', arg, 0, perr, 0)
	...
	!call mnexcm(fcn, 'HESse', (/ 2._dp /), 0, perr, 0)
	call mnexcm(fcn,'MINimize',    (/10000.0_dp,1.0d-5/),2,perr,1)  ! perform MIGRAD minimization
!	do n=1,7
...
!		/ (cosh(gammaV * s**rhoV) - cos(alphaV * s**rhoV + betaV)) )
	...
	!
		...
		call dqagi(f_DVV,s/2._dp,1,1.0d-18,1.0d-40,ni_DV,fabserr,fneval, &
		fier,100,flenw,flast,fiwork,fwork)
...
!	END FUNCTION Chi2FOV
	...
	SUBROUTINE fcn(npar, grad, fval, xval, iflag, futil)
		...
		integer :: npar, iflag, nte, futil
		!external :: futil
		real (kind=dp) :: grad(0:6), xval(0:6), fval
		real (kind=dp), dimension(binmin:78) :: VFO
		real (kind=dp) :: a,b,c,d,c1,d1,atau,chi2FOV,s,i_DVV
		...
		if(futil .eq. 0) then
			...
			do nte = binmin, 78
				VFO(nte) = IntegralPTFO(nte, atau) - ni_DVV(nte, a,b,c,d,c1,d1) - FESR(nte)
			end do
		
		else
		
			do nte = binmin, 78
				s=sbin(nte)+0.5_dp*dsbin(nte)
				i_DVV = exp(-a-b*s)*(d*cos(c+d*s)+b*sin(c+d*s))/(b**2+d**2)
				VFO(nte) = IntegralPTFO(nte, atau) - i_DVV - FESR(nte)
				write(*,*) VFO(nte)
			end do
		
		endif		
		
		Chi2FOV = dot_product(VFO(binmin:78),matmul(SigInvV(binmin:78,binmin:78),VFO(binmin:78)))		
