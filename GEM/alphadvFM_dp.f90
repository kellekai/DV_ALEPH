PROGRAM main
	...
	integer			:: n,m, nn,state, nbin, bestbin, EVierr, NPARI, NPARX, istat  ! used in loops
...
!	call mnexcm(fcn,'SET PRINTout', (/1.0_dp/),1,perr,0)  ! set print level (2=full)
	...
	call MNparm(1,'alpha',3.5d0,0.1d0,0.0d0,0.0d0,perr)
	call MNparm(2,'beta',0.6d0,0.1d0,0.0d0,0.0d0,perr)
	call MNparm(3,'gamma',11.0d0,0.1d0,0.0d0,0.0d0,perr)
	call MNparm(4,'delta',-4.3d0,0.1d0,0.0d0,0.0d0,perr)
...
!	call mnexcm(fcn, 'SCAn', (/ 6._dp, 40.0d0,pval(6) - 0.1d0*pval(6),pval(6) + 0.1d0*pval(6)  /), 4, perr, 0)
	...
	call get_ERR(5, pval, ierr, fval, 1.0d-4)	

	write(*,*) '=== NOT MINUIT ERRORS -4 ==='
...
!		s = sbin(bin) + dsbin(bin) / 2.0_dp
	...
	SUBROUTINE get_ERR(npar, pa, pe, func, eps)
		...
		do n = 1, npar
			...
			fF		= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5))
			pa(n)	= dum(n) * (1.0d0 - eps)
			fB		= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5))
			...
			do m = n+1 , npar
				...
				fFfF	= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5))
				pa(n)	= dum(n) * (1.0d0 - eps)
				pa(m)	= dum(m) * (1.0d0 - eps)
				fBfB	= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5))
				pa(n)	= dum(n) * (1.0d0 + eps)
				pa(m)	= dum(m) * (1.0d0 - eps)
				fFfB	= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5))
				pa(n)	= dum(n) * (1.0d0 - eps)
				pa(m)	= dum(m) * (1.0d0 + eps)
				fBfF	= Chi2FOV(pa(1),pa(2),pa(3),pa(4),pa(5))
