PROGRAM main
	...
	open(95, file='RESULT_GEM_CIPT', status='REPLACE')
	write(95,*) 'DOF, binmin, Energy, (Pval,Perr)[1->7], Chi, P-Value, Converged [0=yes]'
	
	call MNparm(1,'alphaV',4.064d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(2,'betaV',2.6d-4,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(3,'gammaV',5.02d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(4,'deltaV',0.072d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(5,'rhoV',9.18d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(6,'sigmaV',4.01d0,0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(7,'atau',0.336d0,0.1d-4,0.0d0,0.0d0,perr)	
...
!	call mnexcm(fcn,'SET PRINTout', (/1.0_dp/),1,perr,0)  ! set print level (2=full)
	...
	call MNparm(1,'alphaV',pval(1),0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(2,'betaV',pval(2),0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(3,'gammaV',pval(3),0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(4,'deltaV',pval(4),0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(5,'rhoV',pval(5),0.1d-4,0.0d0,0.0d0,perr)
	call MNparm(6,'sigmaV',pval(6),0.1d-4,0.0d0,0.0d0,perr)
